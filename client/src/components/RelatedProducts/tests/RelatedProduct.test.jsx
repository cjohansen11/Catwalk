...
describe('Related products section', () => {
  ...
  it('should display titles for Related Products and Your Outfit', () => {
    render(<RelatedProduct featuredProduct={sampleData} ></RelatedProduct>);
    expect(screen.queryByText(/related products/i)).toBeTruthy();
    expect(screen.queryByText(/your outfit/i)).toBeTruthy();
  });

  it('should have render two figures', () => {
    render(<RelatedProduct featuredProduct={sampleData} ></RelatedProduct>);
    expect(screen.getAllByRole(/figure/i).length).toBe(2);
  });
});